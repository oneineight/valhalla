...
void AddToGraph(GraphTileBuilder& tilebuilder_transit,
  ...
  for (const auto& stop_edges : stop_edge_map) {
    ...
    if (added_stations.find(platform.prev_type_graphid()) == added_stations.end()) {
      ...
      NodeInfo station_node(base_ll, station_ll, n_access, true, NodeType::kTransitStation,
                            false);
      ...
      while (true) {
        ...
        PointLL base_ll = tilebuilder_transit.header_builder().base_ll();
        NodeInfo egress_node(base_ll, egress_ll, n_access, true, NodeType::kTransitEgress, false);
    ...
    PointLL base_ll = tilebuilder_transit.header_builder().base_ll();
    NodeInfo platform_node(base_ll, platform_ll, n_access, true,
